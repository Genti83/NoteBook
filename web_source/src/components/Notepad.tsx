...
export function Notepad() {
  ...
  const askAi = async (overridePrompt?: string) => {
    ...
    setAiChatResponse('');
    try {
       // Ndërto dokumentin aktiv pa imazhet (base64) për të mos ngarkuar bandwidth dhe mudelin
       const activeDoc = documents.find(d => d.id === activeDocId);
       const docsForAi = activeDoc ? [{
          ...activeDoc,
          rows: activeDoc.rows.map(r => {
             const { image, ...rest } = r;
             return rest;
          })
       }] : [];
  ...
  const triggerAutoSave = (updatedDocs: GridDocument[]) => {
      if (typeof window !== 'undefined') {
         if ((window as any)._localSaveTimeout) clearTimeout((window as any)._localSaveTimeout);
         (window as any)._localSaveTimeout = setTimeout(() => {
          ...
          if (downloadMethod === 'folder') {
              ...
              if (rootHandle) {
                  try {
                      ...
                      showToast(`U ruajt në dosjen: ${rootHandle.name}`);
                      return;
                  } catch (e: any) {
                      console.error(e);
                      showToast("Gabim gjatë ruajtjes në dosje.");
                  }
              } else {
